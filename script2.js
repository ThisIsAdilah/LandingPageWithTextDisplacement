...
const config = {
  logoPath: "logo.jpg",  // JPG version that works
  logoSize: 1250,
...
function render() {
  ...
  let bgColor;
  if (config.canvasBg.startsWith('rgba')) {
    const rgba = config.canvasBg.match(/rgba\((\d+),\s*(\d+),\s*(\d+),\s*([\d.]+)\)/);
    if (rgba) {
      bgColor = {
    ...
    const hex = hexToRgb(config.canvasBg);
    bgColor = { r: hex.r, g: hex.g, b: hex.b, a: 1.0 };
  }
